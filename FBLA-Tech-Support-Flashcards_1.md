...
<details>
<summary>Name the main internal PC components and each one's job.</summary>

CPU (processing), motherboard (connects everything), RAM (fast temporary working memory), GPU (graphics rendering), storage HDD/SSD (permanent data), PSU (power), cooling (heatsink/fan/liquid).
</details>

<details>
<summary>GPU: integrated vs discrete?</summary>

Integrated = built into the CPU/motherboard, shares system RAM, lower power. Discrete = a separate card with its own **VRAM**, far more powerful for gaming, rendering, and AI workloads.
</details>

<details>
<summary>IPv4 vs IPv6?</summary>

IPv4 = 32-bit, dotted decimal (e.g., 192.168.1.1), ~4.3 billion addresses. IPv6 = 128-bit, hexadecimal (e.g., 2001:db8::1), effectively unlimited addresses.
</details>

<details>
<summary>What do a subnet mask and default gateway do?</summary>

The **subnet mask** separates the network portion from the host portion of an IP (e.g., /24 = 255.255.255.0). The **default gateway** is the router IP a device sends traffic to when the destination is on a different network.
</details>

<details>
<summary>What does a modem do, vs a router?</summary>

A **modem** connects your network to the ISP, converting signals to/from the internet line (cable/DSL/fiber). A **router** then shares that connection among devices and routes traffic. Many home units combine both.
</details>

<details>
<summary>What do you check for a program's software requirements before installing?</summary>

CPU architecture (32- vs 64-bit), minimum RAM, free storage space, OS version/compatibility, and sometimes GPU/driver requirements.
</details>
...
<details>
<summary>Why do help/service desk teams matter to an organization?</summary>

They're the single point of contact for IT issues — minimizing downtime, keeping employees productive, improving user satisfaction, tracking IT health via metrics, and freeing up specialists by resolving common issues at the front line.
</details>
...
<details>
<summary>Remote access tools besides SSH and RDP?</summary>

VNC (cross-platform graphical remote control) and Telnet (remote CLI but **unencrypted — insecure, avoid**). A VPN often provides the secure tunnel these run through.
</details>
